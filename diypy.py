...
location_with_pincode = st.sidebar.selectbox('Select the location', df['location_with_pincode'].unique().tolist(), index=None)
...
if st.sidebar.button("Recommend"):
    ...
    df['combined_features'] = df[['location_with_pincode', 'bedrooms', 'transaction_type', 'property_type', 'price', 'sqft']].apply(
    ...
    if len(filter_data) < 10:
        ...
        st.header(f"There are {len(filter_data)} such apartments available, here are those apartments and some other recommendations")
        ...
        for i, row in rec_data.iterrows():
            ...
            with col2:
                ...
                st.markdown(f"**Sqft:** {row['sqft']} sqft")
                property_link = row['links']
                st.markdown(f"[Click here]({property_link})")
                st.markdown("---")
    if len(filter_data) == 10:
        st.header(f"Here are the recommendations")
        col1, col2, col3 = st.columns(3)
